...
maxDelay = 12;  % Maximum delay (top speed)
...
platSteps = [48]; % Plateau durations
...
for h = 1:length(platSteps)
    ...
    fprintf('%.6g, ', round(delays(1:end-1)*1000));  % Print all but last element
    fprintf('%.6g};\n', round(delays(end)*1000));  % Print the last element
